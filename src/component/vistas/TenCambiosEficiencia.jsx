...
/*  const [datosCambiEfi, setDatosCambiEfi] = useState([]);
  const [error, setError] = useState('');   
                                     //  Para la eficiencia  //
  useEffect(() => {
    const obtenerDatos = async () => {
      try { const resultado = await axios.get('/api/user'); 
      if (!resultado.data || resultado.data.length === 0) { throw new Error('No se obtuvieron datos de la API'); }   
        setDatosCambiEfi(resultado.data);
      } catch (error) {
  ...
  /*    if (!datosCambiEfi) {  return <p>Cargando...</p>; }  */
...
/*  if (error) { return <div>{error}</div>; }  */
  ...
  const cambioEficiencia = [
    { rut: '35.754.246-6', eficiencia: '+17%'},
    { rut: '35.754.246-6', eficiencia: '+12%'},
    { rut: '35.754.246-6', eficiencia: '+15%'},
    { rut: '35.754.246-6', eficiencia: '+8%'},
    { rut: '35.754.246-6', eficiencia: '+10%'},
    { rut: '35.754.246-6', eficiencia: '+3%'},
    { rut: '35.754.246-6', eficiencia: '+5%'},
    { rut: '35.754.246-6', eficiencia: '+1%'},
    { rut: '35.754.246-6', eficiencia: '+2%'},
    { rut: '35.754.246-6', eficiencia: '-1%'},
    { rut: '35.754.246-6', eficiencia: '-2%'},
    { rut: '35.754.246-6', eficiencia: '-4%'},
    { rut: '35.754.246-6', eficiencia: '-5%'},
    { rut: '35.754.246-6', eficiencia: '-6%'},
    { rut: '35.754.246-6', eficiencia: '-7%'},
    { rut: '35.754.246-6', eficiencia: '-8%'},
    { rut: '35.754.246-6', eficiencia: '+3%'},
    { rut: '35.754.246-6', eficiencia: '+5%'},
    { rut: '35.754.246-6', eficiencia: '-2%'},
    ...
    const ordenDatoscambioEficiencia = cambioEficiencia.slice(-24).reverse(); // Obtener los últimos 24 datos y revertir el orden 
  useEffect(() => {
    ...
    const lastDate = new Date(rendimientosCosecha[rendimientosCosecha.length - 1].fecha.split('-').reverse().join('/'));
        // Calcular el día de la semana de la última fecha (0: domingo, 1: lunes, ..., 6: sábado)
    ...
    if (lastDayOfWeek === 0) {
        startOfCurrentWeek = new Date(lastDate.getTime() - 6 * oneDay); // Ajuste lunes sea el primer día
    }
        // Calcular fechas de inicio y fin de la semana actual y la semana anterior
    ...
    const currentWeekData1area = rendimientosCosecha.filter(item => {
        ...
        // Filtrar los datos para la semana anterior
    ...
    const descargarReporte = () => {
      ...
      */
      ...
      const descargarReporte = () => {
        ...
        if (formatoDescarga === 'pdf') {
            const doc = new jsPDF();
            doc.text('Reporte de Eficiencia', 20, 10); 
            let y = 20;
            cambioEficiencia.slice(-24).reverse().forEach(item => {
                doc.text(`RUT: ${item.rut}, Eficiencia: ${item.eficiencia}`, 20, y);
                y += 10;
            });
            doc.save(`reporte_eficiencia_${fechaFormateada}.pdf`);
        } else if (formatoDescarga === 'csv') {
            const csvData = [['RUT', 'Eficiencia']];
            cambioEficiencia.slice(-24).reverse().forEach(item => {
                csvData.push([item.rut, item.eficiencia]);
            });
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `reporte_eficiencia_${fechaFormateada}.csv`;
            link.click();
        } else { alert('Por favor selecciona un formato de descarga válido (pdf o csv).'); }
    };
    ...
    <div>
      ...
      <div id='layoutSidenav' className={` ${sidebarVisible ? '' : 'layoutSidenav_nav-hidden'}`}>
        ...
        <div id="layoutSidenav_content" className={`${sidebarVisible ? '' : 'layoutSidenav_content-shifted'}`}>
            <div>
                <h4 className='tituladmin'>Cambios en la Eficiencia </h4>
                <br/>
                        ...
                        <div className="tablivilil3 tablitaCambiEfi" >
                            ...
                            <button className='btn nav-link mb-2 separound_spas buttondescargar'  
                             onClick={descargarReporte} >  <FaFileDownload  /> Descargar  </button>
                                 <h5 className='tit_lis_oper' >Eficiencia </h5>                               
                            </div>
                            <table >
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    {ordenDatoscambioEficiencia.map((item, index) => (
                                        <tr key={index} >
                                            <td className='displa_flex separound_spas'>
                                                <div className=' efiloghome isquier  '><FaHome /></div>
                                                <div className=' derech' > {item.rut} </div>
                                            </td>
                                            <td> Rendimiento Semanal = {item.eficiencia} </td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
...
export default TenCambiosEficiencia;
