...
function LoginAdmin ()  {
  ...
  // esto es para que en ves de cargar http://localhost:3000/LoginCosechador se vea y rediriga a http://localhost:3000/LoginCosechador/
  //pues agrega (/) al final de http://localhost:3000/LoginCosechador
  ...
  const esPasswordValido = (password) => {
    // Implementa la lógica de validación de la contraseña
    ...
    <div >    
      <form className='borderLogin' onSubmit={handleSubmit}> 
        <div>
          ...
          <input className='form-control mb-3  ' type="text" placeholder=" Ingresar Rut "  maxLength={12}   value={rut} onChange={handleChange} /* onChange={e => setRut(e.target.value)} */ autoComplete="rut" /* required */  />  
          <p>{rut.length} </p>
        </div>
        <div>
          <label className='mb-2' >  Contraseña:  </label>  
          <input className='form-control mb-3 ' type="password" placeholder= "Ingresar contraseña"  maxLength={14}   value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" /*   required  */ />
          <p>{password.length} </p>  
        </div>
